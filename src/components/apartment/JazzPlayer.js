...
export default function JazzPlayer({ player, tracks, content }) {
  ...
  return <>
    ...
    <p className="eyebrow" id="jazz-choices">{content.choicesLabel}</p>
    <ol className="jazz-tracks" aria-labelledby="jazz-choices">
      {tracks.map((item, index) => <li key={item.id}>
    ...
    <p className="jazz-credits">{content.creditsPrefix} <a href={content.artistUrl} target="_blank" rel="noopener noreferrer">{content.artistLabel}</a>, {content.licensePrefix} <a href={content.licenseUrl} target="_blank" rel="noopener noreferrer">{content.licenseLabel}</a>.</p>
  </>;
}
